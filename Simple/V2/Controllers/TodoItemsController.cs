...
namespace Simple.V2.Controllers
{
    ...
    public class TodoItemsController : ControllerBase
    {
        ...
        [HttpGet("{id:long}", Name = nameof(GetById))] // GET: api/TodoItems/5
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(typeof(TodoItem), StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [AllowAnonymous]
        // GET ~/api/v2/TodoItems/{id}
        // GET ~/api/TodoItems/{id}?api-version=2.0
        public async Task<IActionResult> GetById(long id, ApiVersion apiVersion)
        {
            var todoItem = await Context.TodoItems.FindAsync(id);
            if (todoItem == null) return NotFound();
            var result = new { TodoItem = todoItem, ApiVersion = apiVersion };
            return Ok(result);
        ...
        public async Task<ActionResult<TodoItem>> PostTodoItem(TodoItem todoItem, CancellationToken ct = default)
        {
            ...
            //return CreatedAtAction(/*"GetById"*/nameof(GetById), new { id = todoItem.Id }, todoItem);
            return CreatedAtAction(/*"GetById"*/nameof(GetById), new {/* version = "2"*/ version = HttpContext.GetRequestedApiVersion().ToString(), controller = "TodoItems", id = todoItem.Id }, todoItem);
            //return Created($"https://localhost:5001/v1/TodoItems/{todoItem.Id}", todoItem);
        }

        // GET ~/api/v2/GetApiVersion
        // GET ~/api/GetApiVersion?api-version=2.0
        [Route(nameof(GetApiVersion))]
        [AllowAnonymous]
        public ActionResult<ApiVersion> GetApiVersion(ApiVersion apiVersion) => Ok(apiVersion);
